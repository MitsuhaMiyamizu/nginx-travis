...
wget http://nginx.org/download/nginx-1.9.15.tar.gz && tar zxvf nginx-1.9.15.tar.gz
#PageSpeed
...
unzip nginx-ct.zip
cd nginx-1.9.15
./configure --add-module=../nginx-ct-1.2.0 --with-openssl=../openssl --user=www-data --group=www-data --with-http_ssl_module --with-http_v2_module --with-ipv6 --add-module=../ngx_pagespeed-release-1.11.33.2-beta --with-stream 
make
