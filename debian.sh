...
#PageSpeed
wget https://github.com/pagespeed/ngx_pagespeed/archive/release-1.10.33.5-beta.zip -O pagespeed.zip
unzip pagespeed.zip
cd ngx_pagespeed-*
wget https://dl.google.com/dl/page-speed/psol/1.10.33.5.tar.gz
tar -xzvf 1.10.33.5.tar.gz
cd ..
#ModSecurity
...
wget -O openssl.zip -c https://github.com/openssl/openssl/archive/OpenSSL_1_0_2f.zip
unzip openssl.zip
mv openssl-OpenSSL_1_0_2f/ openssl
...
unzip nginx-ct.zip
cd nginx-1.9.12
./configure --add-module=../nginx-ct-1.0.0 --with-openssl=../openssl --user=www-data --group=www-data --with-http_ssl_module --with-http_v2_module --with-ipv6 --add-module=../ngx_pagespeed-release-1.10.33.5-beta --with-stream 
make
